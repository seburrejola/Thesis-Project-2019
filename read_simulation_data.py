...
molecular_rad=0.0    #Minimum radius for a cluster measurement.
...
for step in range(len(lineas)):
    ...
    old_is=[]
    new_is=[]
    caf.set_old_new_is(len(lpos2[0]),old_is,new_is,'old')
    if step>0:
        caf.clustering_check1(len(lpos2[0]),lpos2,old_is,molecular_rad)
    ...
    caf.set_old_new_is(N,old_is,new_is,'new')
    cont1=0
    cont2=0
    for element in linea:
        if element!='|':
            if element not in colors_list:
                lpos2[cont1].append(float(element))
                cont2+=1
            if element in colors_list:
                cont2+=1
                lpos2[cont1].append(element)
        if cont2==N:
            cont1+=1
            cont2=0
    ...
    if step>0:
        ...
        cl=[]
        caf.clusters(len(old_is),lpos2,molecular_rad,cl,old_is,new_is)
           
        caf.save_cluster(cl,ccl,step)       
    ...
    if len(cl_means)!=0:
        ...
        scat = ax2.scatter(Final_plot[0],Final_plot[1],c=Final_plot[2],s=Final_plot[3],alpha=0.50)
    ...
    if len(cl_means)==0:
        ...
        scat = ax2.scatter(Final_plot[0],Final_plot[1],c=Final_plot[2],s=Final_plot[3],alpha=1)  
...
size of cluster in the whole simulation (in magenta).

molecular_rad needs to be bigger than zero.
'''       
#frequency_plot=caf.save_freq_plot2(frequency_list)
#
frequency_plot2=caf.save_freq_plot3(ccl)
#
plt.figure()
#plt.subplot(2,1,1)
...
#plt.subplot(2,1,2)
plt.title('Frequency Plot')
plt.hist(frequency_plot2,log=True,bins=150,facecolor='magenta')#'bs'
plt.grid(True)   
